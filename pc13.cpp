...
string* makeDynoString (string contents){
	string* dynoContents= NULL;
	dynoContents = new string(contents);
	return dynoContents;
}
...
bool findWord (string* theString, string theWord){
	if (theString == NULL){
		...
		size_t found = theString->find(theWord);
...
bool replaceWord (string* theString, string oldWord, string newWord){
	if (theString == NULL){
		...
		size_t found = theString->find(oldWord);
			if (found!=string::npos){
				theString->replace(found, oldWord.length(), newWord);
				return true;
		}
			else{
				return false;
