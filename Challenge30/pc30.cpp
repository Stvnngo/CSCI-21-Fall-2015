...
/**
		...
		bool HashTable::remove (string contents){
			...
			// 	return false;
			// }
			// items[hash(contents)].erase(*(getItem(contents)));
			// return true
			for(list<Item>::iterator i = items[hash(contents)].begin(); i != items[hash(contents)].end(); i++){
				if(i->contents == contents){
					items[hash(contents)].erase(i);
					return true;
				}
			}
			return false;
		...
		string HashTable::printDetail (){
			...
			for(unsigned int i = 0; i != 5; i++){
					ss << "[" << i << "]" << '\t';
					for(list<Item>::iterator i2 = items[i].begin(); i2 != items[i].end(); i2++){ 
						ss << setw(10) << i2->contents << " (" << i2->count << ") ";
					}
					ss << endl;
			}
			return ss.str();
